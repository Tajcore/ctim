...
from crew.models import AgentModel, CrewModel, TaskModel, ToolModel, ToolRegistryModel
...
class CrewModelTests(TestCase):
    def setUp(self):
        ...
        self.crew = CrewModel.objects.create(name="Crew1", status="Active")
    ...
    def test_missing_name(self):
        crew = CrewModel(status="Active")
        with self.assertRaises(ValidationError):
            crew.full_clean()

    def test_default_status(self):
        crew = CrewModel.objects.create(name="Crew2")
    ...
    def test_long_strings_for_name_status(self):
        long_string = "a" * 100
        crew = CrewModel.objects.create(name=long_string, status=long_string[:50])
        try:
            crew.full_clean()
        except ValidationError:
            self.fail("Crew model raised ValidationError unexpectedly!")
    ...
    def test_assign_empty_agents_and_tasks(self):
        crew = CrewModel.objects.create(name="Crew3", status="Active")
        crew.agents.set([])
        crew.tasks.set([])
    ...
    def test_create_crew_with_agents_and_tasks(self):
        ...
        crew = CrewModel.objects.create(name="Crew4", status="Active")
